...
describe Entry::Scraper do
  ...
  context 'winner' do
    before do
      entry_content = File.read("#{Dir.pwd}/spec/fixtures/winner.html")
  ...
  context 'entrant' do
    before do
      entry_content = File.read("#{Dir.pwd}/spec/fixtures/entrant.html")
      FakeWeb.register_uri(:get, entry_url, body: entry_content, content_type: 'text/html')
    end
    ...
    describe '#get_entry' do
      ...
      it "saves award as 'entry'" do
        scraper.entries[0].award.should eq 'entry'
      end
    end
  end
end